...
if not st.session_state.calculated:
    ...
    if not res.empty:
        ...
        if not st.session_state.unlocked:
            ...
            with tab1:
                st.markdown("### 感情與人際")
                st.write(data.get('Content_Love', '無資料'))
            with tab2:
                st.markdown("### 事業與工作")
                st.write(data.get('Content_Career', '無資料'))
            with tab3:
                st.markdown("### 財運與投資")
                st.write(data.get('Content_Fortune', '無資料'))
            with tab4: 
                st.markdown("### 🏥 健康與平安")
                st.write(data.get('Content_Health', '無資料'))
            with tab5:
